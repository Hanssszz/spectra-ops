  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < e.target.files.length; i++) {
        ...
        const res = await api.post('/incidents/upload', data, {
        ...
        async (pos) => {
          ...
          try {
            await api.post('/incidents', {
              ...formData,
              reporterId: user?.id || 'guard_id_mock',
        ...
        <div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            ...
            <button 
              ...
              className="shrink-0 h-24 w-24 rounded-xl border-2 border-dashed border-border flex flex-col items-center justify-center gap-1 text-muted-foreground hover:bg-secondary/50 hover:text-foreground transition-colors disabled:opacity-50"
            >
              <Camera className="h-6 w-6" />
              <span className="text-[10px] font-medium">{isUploading ? 'Uploading...' : 'Capture'}</span>
            </button>
            <button 
              type="button" 
              onClick={() => document.getElementById('media-upload')?.click()}
            ...
            {mediaUrls.map((url, i) => (
              <div key={i} className="relative shrink-0 h-24 w-24 rounded-xl overflow-hidden border border-border group bg-secondary">
